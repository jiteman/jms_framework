  ...
  switch (mt->policy)
	...
	case MT_SUBALLOC:
	  if (mt->tail == NULL
		...
		new_block (mt, (size_t)mt->arg[MT_BLOCK_SIZE]);
	  if (mt->tail->used + size <= (size_t) mt->arg[MT_BLOCK_SIZE])
		{
		  void *p = (char *) mt->tail->content + mt->tail->used;
		  size = ((size + (size_t)mt->arg[MT_ALIGN] - 1)
				  / (size_t)mt->arg[MT_ALIGN] * (size_t)mt->arg[MT_ALIGN]);
		  mt->tail->used += size;
		  if (mt->verbosity >= 3)
...
int test_main( int argc, char *argv[] ) {
  ...
  insert = xmalloc ( sizeof *insert * (size_t)opts.node_cnt);
  delete = xmalloc ( sizeof *delete * (size_t)opts.node_cnt);
  ...
  while (opts.iter_cnt--)
	{
	  ...
	  srand (opts.seed);
	  gen_insertions ((size_t)opts.node_cnt, opts.insert_order, insert);

	  srand (++opts.seed);
	  gen_deletions ((size_t)opts.node_cnt, opts.delete_order, insert, delete);
