...
#include <limits.h>
#include <stdio.h>
#include "jmsf/structing/tavl.h"
#include "test.h"

/* Prints the structure of |node|,
   which is |level| levels from the top of the tree. */
void print_tree_structure( struct tavl_node *node, int level );
void print_tree_structure( struct tavl_node *node, int level ) {
  int i;
...
static void print_whole_tree( const struct tavl_table *tree, const char *title ) {
  printf ("%s: ", title);
