...
static void print_whole_tree( const struct avl_table *tree, const char *title ) {
	printf( "%s: ", title );
...
static int test_correctness (struct libavl_allocator *allocator,
  ...
  for (i = 0; i < n; i++)
	{
	  ...
	  if ( !verify_tree( tree, insert, ( size_t )( i + 1 ) ) )
		return 0;
	}
  ...
  for (i = 0; i < n; i++)
	{
	  ...
	  if ( !verify_tree( tree, insert, ( size_t)( n ) ) )
		return 0;
	}
  ...
  for (i = 0; i < n; i++)
	{
	  ...
	  if ( !verify_tree( tree, delete + i + 1, ( size_t )( n - i - 1 ) ) )
		return 0;
...
static int test_bst_t_first( struct avl_table *tree, int n ) {
	struct avl_traverser trav;
	int *first;
...
static int test_bst_copy( struct avl_table *tree, int n ) {
	struct avl_table *copy = avl_copy( tree, NULL, NULL, NULL );
  ...
  for (i = test; i < test + sizeof test / sizeof *test; i++)
	{
	  ...
	  if ( verify_tree( tree, order, ( size_t )( n ) ) == 0 )
		return 0;
	  avl_destroy (tree, NULL);
	}
