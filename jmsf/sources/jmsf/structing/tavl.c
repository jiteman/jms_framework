...
void **tavl_probe( struct tavl_table *tree, void *item ) {
	struct tavl_node *top_node_to_update_balance_factor; /* Top node to update balance factor, and parent. */
	struct tavl_node *top_node_to_update_balance_factor_and_parent; /* Top node to update balance factor, and parent. */
	struct tavl_node *iterator; /* Iterator, and parent. */
	struct tavl_node *iterator_and_parent; /* Iterator, and parent. */
	struct tavl_node *newly_inserted_node;     /* Newly inserted node. */
	struct tavl_node *new_root_of_rebalanced_subtree;     /* New root of rebalanced subtree. */
	int direction_to_descend;                /* Direction to descend. */

//  unsigned char da[TAVL_MAX_HEIGHT]; /* Cached comparison results. */
	int cached_comparation_results[ TAVL_MAX_HEIGHT ]; // Cached comparison results.
	int number_of_cached_results = 0;              /* Number of cached results. */

	assert( tree != NULL && item != NULL );

	top_node_to_update_balance_factor_and_parent = ( struct tavl_node * )&tree->tavl_root;
	top_node_to_update_balance_factor = tree->tavl_root;

	if ( top_node_to_update_balance_factor != NULL ) {
		for (
			iterator_and_parent = top_node_to_update_balance_factor_and_parent, iterator = top_node_to_update_balance_factor;
			;
			iterator_and_parent = iterator, iterator = iterator->tavl_link[ direction_to_descend ] )
		{
			int cmp = tree->tavl_compare( item, iterator->tavl_data, tree->tavl_param );

			if ( cmp == 0 ) {
				return &iterator->tavl_data;
			}

			if ( iterator->tavl_balance != 0 ) {
				top_node_to_update_balance_factor_and_parent = iterator_and_parent;
				top_node_to_update_balance_factor = iterator;
				number_of_cached_results = 0;
			}

			cached_comparation_results[ number_of_cached_results++ ] = ( direction_to_descend = ( cmp > 0 ) );

			if ( iterator->tavl_tag[ direction_to_descend ] == TAVL_THREAD) {
				break;
			}
		}
	} else {
		iterator = top_node_to_update_balance_factor_and_parent;
		direction_to_descend = 0;
	}

	newly_inserted_node = tree->tavl_alloc->libavl_malloc( tree->tavl_alloc, sizeof *newly_inserted_node );

	if ( newly_inserted_node == NULL ) return NULL;

	tree->tavl_count++;
	newly_inserted_node->tavl_data = item;
	newly_inserted_node->tavl_tag[ 0 ] = newly_inserted_node->tavl_tag[ 1 ] = TAVL_THREAD;
	newly_inserted_node->tavl_link[ direction_to_descend ] = iterator->tavl_link[ direction_to_descend ];

	if (tree->tavl_root != NULL) {
		iterator->tavl_tag[ direction_to_descend ] = TAVL_CHILD;
		newly_inserted_node->tavl_link[ !direction_to_descend ] = iterator;
	} else {
		newly_inserted_node->tavl_link[ 1 ] = NULL;
	}

	iterator->tavl_link[ direction_to_descend ] = newly_inserted_node;
	newly_inserted_node->tavl_balance = 0;

	if ( tree->tavl_root == newly_inserted_node ) {
		return &newly_inserted_node->tavl_data;
	}

	for ( iterator = top_node_to_update_balance_factor, number_of_cached_results = 0; iterator != newly_inserted_node; iterator = iterator->tavl_link[ cached_comparation_results[ number_of_cached_results ] ], number_of_cached_results++ ) {
		if ( cached_comparation_results[ number_of_cached_results ] == 0 ) {
			iterator->tavl_balance--;
		} else {
			iterator->tavl_balance++;
		}
	}

	if ( top_node_to_update_balance_factor->tavl_balance == -2 ) {
		struct tavl_node *x = top_node_to_update_balance_factor->tavl_link[ 0 ];

		if ( x->tavl_balance == -1 ) {
			new_root_of_rebalanced_subtree = x;

			if ( x->tavl_tag[ 1 ] == TAVL_THREAD ) {
				x->tavl_tag[ 1 ] = TAVL_CHILD;
				top_node_to_update_balance_factor->tavl_tag[ 0 ] = TAVL_THREAD;
				top_node_to_update_balance_factor->tavl_link[ 0 ] = x;
			} else
				top_node_to_update_balance_factor->tavl_link[ 0 ] = x->tavl_link[ 1 ];

			x->tavl_link[ 1 ] = top_node_to_update_balance_factor;
			x->tavl_balance = top_node_to_update_balance_factor->tavl_balance = 0;
		} else {
			assert( x->tavl_balance == +1 );
			new_root_of_rebalanced_subtree = x->tavl_link[ 1 ];
			x->tavl_link[ 1 ] = new_root_of_rebalanced_subtree->tavl_link[ 0 ];
			new_root_of_rebalanced_subtree->tavl_link[ 0 ] = x;
			top_node_to_update_balance_factor->tavl_link[ 0 ] = new_root_of_rebalanced_subtree->tavl_link[ 1 ];
			new_root_of_rebalanced_subtree->tavl_link[ 1 ] = top_node_to_update_balance_factor;

			if ( new_root_of_rebalanced_subtree->tavl_balance == -1 ) {
				x->tavl_balance = 0;
				top_node_to_update_balance_factor->tavl_balance = +1;
			} else if (new_root_of_rebalanced_subtree->tavl_balance == 0) {
				x->tavl_balance = top_node_to_update_balance_factor->tavl_balance = 0;
			} else { /* |w->tavl_balance == +1| */
				x->tavl_balance = -1;
				top_node_to_update_balance_factor->tavl_balance = 0;
			}

			new_root_of_rebalanced_subtree->tavl_balance = 0;

			if ( new_root_of_rebalanced_subtree->tavl_tag[ 0 ] == TAVL_THREAD ) {
				x->tavl_tag[ 1 ] = TAVL_THREAD;
				x->tavl_link[ 1 ] = new_root_of_rebalanced_subtree;
				new_root_of_rebalanced_subtree->tavl_tag[ 0 ] = TAVL_CHILD;
			}

			if ( new_root_of_rebalanced_subtree->tavl_tag[ 1 ] == TAVL_THREAD ) {
				top_node_to_update_balance_factor->tavl_tag[ 0 ] = TAVL_THREAD;
				top_node_to_update_balance_factor->tavl_link[ 0 ] = new_root_of_rebalanced_subtree;
				new_root_of_rebalanced_subtree->tavl_tag[ 1 ] = TAVL_CHILD;
			}
		}
	} else if ( top_node_to_update_balance_factor->tavl_balance == +2 ) {
		struct tavl_node *x = top_node_to_update_balance_factor->tavl_link[ 1 ];
		...
		if ( x->tavl_balance == +1 ) {
			...
			if ( x->tavl_tag[ 0 ] == TAVL_THREAD ) {
				...
				top_node_to_update_balance_factor->tavl_tag[ 1 ] = TAVL_THREAD;
				top_node_to_update_balance_factor->tavl_link[ 1 ] = x;
			} else
				top_node_to_update_balance_factor->tavl_link[ 1 ] = x->tavl_link[ 0 ];

			x->tavl_link[ 0 ] = top_node_to_update_balance_factor;
			x->tavl_balance = top_node_to_update_balance_factor->tavl_balance = 0;
		} else {
			assert( x->tavl_balance == -1 );
			new_root_of_rebalanced_subtree = x->tavl_link[ 0 ];
			x->tavl_link[ 0 ] = new_root_of_rebalanced_subtree->tavl_link[ 1 ];
			new_root_of_rebalanced_subtree->tavl_link[ 1 ] = x;
			top_node_to_update_balance_factor->tavl_link[ 1 ] = new_root_of_rebalanced_subtree->tavl_link[ 0 ];
			new_root_of_rebalanced_subtree->tavl_link[ 0 ] = top_node_to_update_balance_factor;
			...
			if ( new_root_of_rebalanced_subtree->tavl_tag[ 0 ] == TAVL_THREAD ) {
				top_node_to_update_balance_factor->tavl_tag[ 1 ] = TAVL_THREAD;
				top_node_to_update_balance_factor->tavl_link[ 1 ] = new_root_of_rebalanced_subtree;
				new_root_of_rebalanced_subtree->tavl_tag[ 0 ] = TAVL_CHILD;
			}

			if ( new_root_of_rebalanced_subtree->tavl_tag[ 1 ] == TAVL_THREAD ) {
				x->tavl_tag[ 0 ] = TAVL_THREAD;
				x->tavl_link[ 0 ] = new_root_of_rebalanced_subtree;
				new_root_of_rebalanced_subtree->tavl_tag[ 1 ] = TAVL_CHILD;
			}
		}
	} else {
	...
	top_node_to_update_balance_factor_and_parent->tavl_link[ top_node_to_update_balance_factor != top_node_to_update_balance_factor_and_parent->tavl_link[ 0 ] ] = new_root_of_rebalanced_subtree;
	return &newly_inserted_node->tavl_data;
}
...
size_t tavl_count( struct tavl_table *a_table ) {
	return a_table->tavl_count;
}
